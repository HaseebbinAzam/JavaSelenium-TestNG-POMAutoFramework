package com.qa.opencart.errors;

public class AppError {
	
	public static final String BROWSER_NOT_FOUND ="====BROWSER_NOT_FOUND====";
	public static final String TITLE_NOT_FOUND ="====TITLE_NOT_FOUND====";
	public static final String URL_NOT_FOUND ="====URL_NOT_FOUND====";
	public static final String ELEMENT_NOT_FOUND ="====ELEMENT_NOT_FOUND====";
	public static final String LIST_IS_NOT_MATCHED = "====LIST IS NOT MATCHED OR EMPTY====";
	public static final String RESULTS_COUNT_MISMATCHED = "====PRODUCT SEARCH RESULT MISMATCHED====";
	public static final String HEADER_NOT_FOUND = "====HEADER NOT FOUND====";
	public static final String IMAGES_COUNT_MISMATCHED = "====IMAGES COUNT MISMATCHED====";
	public static final String USER_REG_NOT_DONE = "====USER REGISTRATION NOT DONE====";

}
